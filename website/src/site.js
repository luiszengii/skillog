...
const renderContent = (items) => {
  ...
  $("#content-root").innerHTML = items.map((post, postIndex) => {
    ...
    return `<section class="post-record">
      <article class="post-feature">
        <figure class="post-feature__cover">
          <img src="${esc(post.cards[0])}" alt="${esc(post.title)}" width="1080" height="1440" ${postIndex === 0 ? 'fetchpriority="high"' : 'loading="lazy"'} />
        </figure>
        <div class="post-feature__info">
          ...
          <div class="actions">
            <button type="button" data-copy-title data-post-index="${postIndex}">复制标题</button>
            <button type="button" data-copy-body data-post-index="${postIndex}">复制正文</button>
      ...
      <div class="gallery" aria-label="${esc(post.id)} 四张成品图">${gallery}</div>
  ...
  document.querySelectorAll("[data-copy-title]").forEach((button) => {
    const post = items[Number(button.dataset.postIndex)];
    button.addEventListener("click", (event) => copyText(event.currentTarget, post.title, "复制标题"));
  });
  document.querySelectorAll("[data-copy-body]").forEach((button) => {
    const post = items[Number(button.dataset.postIndex)];
    button.addEventListener("click", (event) => copyText(event.currentTarget, post.body, "复制正文"));
  });
};
